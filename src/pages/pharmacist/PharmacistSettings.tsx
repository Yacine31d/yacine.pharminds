...
import { LocationPicker } from '@/components/pharmacist/LocationPicker';
  ...
  const handleSaveLocation = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq('user_id', user.id);
      if (error) throw error;
      toast({ title: 'Localisation enregistrée', description: 'Votre position est maintenant visible sur le Radar Stock.' });
    } catch (err) {
      console.error(err);
      toast({ title: 'Erreur', description: 'Impossible de sauvegarder la localisation.', variant: 'destructive' });
    } finally {
        ...
        <div className="p-4 md:p-8">
          ...
          <Tabs defaultValue="profile" className="space-y-6">
            ...
            <TabsContent value="location">
              <GlowCard className="p-6">
                <div className="mb-6">
                  <h2 className="text-xl font-semibold mb-1">Localisation de la pharmacie</h2>
                  <p className="text-sm text-muted-foreground">
                    Définissez votre position exacte pour apparaître précisément sur la carte Radar Stock des patients.
                  </p>
                </div>
                <LocationPicker
                  lat={coords.lat}
                ...
                <div className="mt-6 flex justify-end">
                  <Button
                    ...
                    disabled={savingLocation || coords.lat === null}
                    className="gap-2"
                  >
                    {savingLocation
                      ? <><Loader2 className="w-4 h-4 animate-spin" /> Enregistrement…</>
                      : <><Save className="w-4 h-4" /> Enregistrer la position</>
                    }
                  </Button>
                </div>
              </GlowCard>
