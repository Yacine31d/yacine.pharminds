...
Rôle de l'utilisateur: ${userRole}. Adapte le niveau de technicité en conséquence.`;
      ...
      const friendlyMsg = msg.includes('401') || msg.includes('403')
        ? '🔑 Clé API invalide ou expirée. Configurez VITE_GEMINI_API_KEY dans Vercel.'
        : msg.includes('429')
        ? '⏱️ Trop de requêtes. Veuillez patienter quelques secondes.'
        : '⚠️ Service temporairement indisponible. Veuillez réessayer.';
      setMessages(prev => {
        const filtered = prev[prev.length - 1]?.content === '' ? prev.slice(0, -1) : prev;
        return [...filtered, { role: 'assistant', content: friendlyMsg }];
      });
    } finally {
      setIsLoading(false);
    }
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    {import.meta.env.VITE_GROQ_API_KEY
                      ...
                      : 'Powered by Pollinations AI'} · En ligne
                  </p>
                </div>
              </div>
