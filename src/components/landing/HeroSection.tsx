...
import { ArrowRight, Shield, Brain, TrendingUp, Stethoscope, User } from 'lucide-react';
...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <motion.div
        className="container relative z-10 px-4 py-20 pt-32"
        style={{
          opacity: heroOpacity,
          scale: heroScale,
          y: heroY,
      ...
      >
        <div className={`max-w-5xl mx-auto text-center ${isRTL ? 'font-arabic' : ''}`}>
          ...
          {/* Admin link — tertiary, less prominent */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <button
              onClick={() => navigate('/auth')}
              className="text-sm text-muted-foreground hover:text-primary transition-colors underline-offset-4 hover:underline flex items-center gap-1"
            >
