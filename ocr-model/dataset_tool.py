...
ARTIFACT_RE = re.compile(
    ...
    r"Patient Name:|Date:|Prescription:|Signature:",
    re.IGNORECASE,
)
...
async function quickFix() {
  ...
  try {
    const d = await fetch('/clean/run', {
      ...
      body: JSON.stringify({steps:['duplicates','arabic','artifacts','multiline']})
    }).then(r=>r.json());
...
<div class="card">
  ...
  <div class="step-row">
    ...
    <div class="step-info">
      ...
      <p>Remplace \\n par espace — labels vides apres correction → trash</p>
    </div>
...
def clean_run():
    ...
    if "arabic" in steps:
        ...
        for r in rows:
            (arabic_rows if ARABIC_RE.search(r.get("text", "")) else clean_rows).append(r)
        if arabic_rows:
...
<script>
let files=[], labels={}, reviewed={}, idx=0;
...
def api_review_state():
    ...
    return jsonify({"files": files, "labels": labels, "reviewed": state})
...
<div class="card">
  ...
  <div class="g2">
    <div class="fg">
      <label>Images printed a generer</label>
      <input type="number" id="nPrinted" value="2000" min="0" max="10000">
    </div>
    <div class="fg">
      <label>Images handwritten a generer</label>
...
<div class="card">
  ...
  <div class="g2">
    ...
    <div class="fg">
      <label>Max printed synthetique</label>
      <input type="number" id="bPr" value="2000" min="0">
    </div>
    <div class="fg">
